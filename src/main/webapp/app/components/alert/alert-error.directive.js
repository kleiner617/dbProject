(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('dbProjectApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-dbProjectApp-error');
                var entityKey = httpResponse.headers('X-dbProjectApp-params');
                if (errorHeader) {
